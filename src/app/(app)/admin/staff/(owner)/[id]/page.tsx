  ...
  const [{ data: staff }, { data: businesses }, { data: tours }, { data: assigned }] =
    await Promise.all([
      supabase
        .from("staff")
        .select(
          "id, full_name, email, role, business_id, is_active, can_create_bookings, can_edit_bookings, can_check_in, can_delete_bookings, can_add_to_peek, can_view_attachments, can_redeem_groupon, can_view_details, can_use_caja, pin_required",
        )
        .eq("id", id)
        .maybeSingle(),
      ...
      {staff.role === "owner" ? (
        ...
        <EditStaffForm
          staff={{
            ...
            can_add_to_peek: staff.can_add_to_peek,
            can_view_attachments: staff.can_view_attachments,
