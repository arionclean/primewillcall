...
backs it. Three are **manager switches** (`can_manage_sales`: refund, void and move
sales; `can_manage_team`: change the People tab; `can_view_payments`: open Payments),
all default on. Owners always have all twelve. See `docs/DATABASE.md`.
...
scripts/                       import_legacy_bookings.py (one-way Xano -> Supabase tunnel);
                               ...
                               here, that is xano_backfill_cash.py's job);
...
- **Public booking page `/booking/<token>`** is built: the link guests get after
  ...
  `bookingConfirmation_id`). The page also opens on `xano_confirmation_token`
  (Xano's code in the `bked.io/booking/<code>` links Xano still texts for the
  bookings it creates, filled hourly by the `xano-ticket-tokens` sweep) and on
  `xano_internal_id`, one query, exactly one match required. So once
  `pro.primewillcall.com` (where the `bked.io` registrar 301 lands, today the
  Bubble app) points at this app, every old ticket link opens here and Bubble can
  go. The legacy page's upsell section is intentionally NOT
