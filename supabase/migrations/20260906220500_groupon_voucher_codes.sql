-- Groupon Redemption Codes get a column of their own.
--
-- Staff redeem a voucher on Groupon by its Redemption Code, and the bookings list now
-- shows that code on the row with a one-click copy. Until now the codes only lived in
-- `bookings.legacy_reference` (comma-joined by gp-book) and in the note, and neither is a
-- stable home. `legacy_reference` is Xano's: when the Xano mirror is on, Xano's "New
-- Supabase platform" trigger sends the booking back through xano-booking-sync, which
-- upserts `legacy_reference` from Xano's `booking_reference`, the `GP-...` mirror
-- reference, so the codes would vanish on the first round trip. The note is free text
-- staff edit. `groupon_voucher_codes` is written once by create_booking() and is not a
-- field xano-booking-sync maps, the same reason `groupon_redeemed_at` has its own column.
--
-- One element per voucher, in upload order. `legacy_reference` keeps the comma-joined
-- copy: it is what Xano and search already read.

alter table public.bookings
  add column groupon_voucher_codes text[] not null default '{}'::text[];

comment on column public.bookings.groupon_voucher_codes is
  'Groupon Redemption Codes of a /gp booking, one per voucher, in upload order. Written by create_booking() from gp-book and never mapped by xano-booking-sync. The bookings list shows them with a one-click copy so staff never retype a code into Groupon.';

-- Backfill the /gp bookings made before this column existed. Only entries shaped like a
-- code are kept: the 6-10 digit Redemption Code, or the VS-XXXX-XXXX-XXXX-XXXX voucher
-- number the earliest rows stored (the same shapes _shared/gp-voucher-code.ts accepts).
-- Xano-synced rows are skipped: their legacy_reference is a Xano booking reference.
...
from (
  ...
  cross join lateral unnest(string_to_array(src.legacy_reference, ',')) with ordinality as u(raw, ord)
  where src.source_channel = 'groupon'
    and src.legacy_id is null
    and src.legacy_reference is not null
    and (btrim(u.raw) ~ '^\d{6,10}$' or btrim(u.raw) ~* '^VS(-[A-Z0-9]{4}){4}$')
  group by src.id
) codes
where b.id = codes.id;

-- create_booking() takes the codes. Drop + create rather than `create or replace`: adding
-- a parameter changes the signature, and `create or replace` would leave the old one
-- behind as a second overload, making every call ambiguous. Both statements run in the
-- migration's transaction, so there is no window where /schedule finds no function.

drop function if exists public.create_booking(
  uuid, date, time, text, text, jsonb, integer, text, text, text, text,
  public.booking_status, text, text, integer, uuid, boolean, boolean, text[]
);
