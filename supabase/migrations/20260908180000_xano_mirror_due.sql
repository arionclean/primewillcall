-- The balance due travels to Xano.
--
-- A desk booking can carry what the guest still owes (bookings.due_cents), and the
-- iPad collects it: it shows "Payment Pending" when Xano's payment_status is
-- "pending" and charges Xano's price. The mirror sent neither, so a booking made
-- here with a balance read "Fully Paid" on the tablet (first seen 2026-09-08). The
-- worker now sends them (xano-mirror.ts balanceFields) and this makes a change to
-- the balance a mirrored field, "due". Same function as
-- 20260908101000_xano_mirror_fields_append.sql otherwise.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    -- The balance due: the iPad shows and collects it (payment_status + price).
    if new.due_cents is distinct from old.due_cents then
      v_fields := array_append(v_fields, 'due');
    end if;
